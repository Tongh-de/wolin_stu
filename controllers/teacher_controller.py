...
def create_teacher(teacher: TeacheresUpdata, db: Session = Depends(get_db)):
    logger.info(f"新增教师: {teacher.teacher_name}, 角色: {teacher.role}")
...
def get_teacher(
    ...
    logger.info(f"查询教师: teacher_id={teacher_id}, teacher_name={teacher_name}")
...
def update_teacher(teacher_id: int, teacher: TeacheresUpdata, db: Session = Depends(get_db)):
    logger.info(f"更新教师: teacher_id={teacher_id}")
...
def delete_teacher(teacher_id: int, db: Session = Depends(get_db)):
    logger.info(f"删除教师: teacher_id={teacher_id}")
...
def bind_teacher_class(
    ...
    logger.info(f"绑定班级: teacher_id={teacher_id}, class_ids={class_ids}")
    try:
...
def unbind_teacher_class(
    ...
    logger.info(f"解除绑定: teacher_id={teacher_id}, class_id={class_id}")
...
def get_head_classes(teacher_id: int, db: Session = Depends(get_db)):
    logger.info(f"查询班主任所带班级: teacher_id={teacher_id}")
...
def get_teach_classes(teacher_id: int, db: Session = Depends(get_db)):
    logger.info(f"查询讲师所教班级: teacher_id={teacher_id}")
...
def get_my_students(teacher_id: int, db: Session = Depends(get_db)):
    logger.info(f"查询顾问所带学生: teacher_id={teacher_id}")
